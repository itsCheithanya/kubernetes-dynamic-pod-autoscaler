...
**Input:** 
- `predicted_workload`  # predicted workload in the next interval in rps
- `current_workload`    # current workload of the application in rps 

**Output:** 
- `Scale UP/DOWN`       # in #pods
...
Resource get_resource_utilization() {
    ...
    for each pod in workload resource (Deployment or StatefulSet) do
        for each container in pod do
            ...
            Avg_Resource1_usage += Resource1_usage
            Avg_Resource2_usage += Resource2_usage
            ...
            Avg_ResourceN_usage += ResourceN_usage
    
    Avg_Resource1_usage /= N
    Avg_Resource2_usage /= N
    ...
    Avg_ResourceN_usage /= N
    
    Resource = get_Resource_utilization_of_bottleneck_resource(Avg_Resource1_usage, Avg_Resource2_usage..., Avg_ResourceN_usage)
    return Resource
}


Resource get_resource_limit() {
    Resource = get_Resource_limits_of_bottleneck_resource(Avg_Resource1_usage, Avg_Resource2_usage..., Avg_ResourceN_usage)
    return Resource
}
